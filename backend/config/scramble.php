...
return [
    ...
    'info' => [
        ...
        'description' => <<<'MD'
        The REST API behind the Foogra restaurant directory.

        ### Authentication

        Sign in at `POST /api/v1/auth/login` and send the returned token as
        `Authorization: Bearer <token>` on every protected call.

        **Demo accounts** — the password for all of them is `password`:

        | Role     | Email                  | Can do                                        |
        |----------|------------------------|-----------------------------------------------|
        | Admin    | `admin@foogra.test`    | Everything, across every restaurant            |
        | Owner    | `owner@foogra.test`    | Manage their own restaurants, menus, bookings  |
        | Customer | `customer@foogra.test` | Book tables, write reviews, keep a wishlist    |

        ### Conventions

        * List endpoints return `{ data: [...], meta: { current_page, last_page, total } }`.
        * Errors always return `{ message: string, errors?: { field: string[] } }`.
        * Restaurants and categories are addressed by **slug**; bookings by **reference**.
        * Auth endpoints allow 10 requests/minute per IP; browsing allows 120.
        MD,
    ],
     ...
     * ],
     */
    // Marks every `auth:sanctum` route as bearer-protected in the spec.
